...
try:
    # None値を-1に変換してJSON化
    send_data = [x if x is not None else -1 for x in result_list]
...
while True:
    ...
    # 結果をJSON形式で送信
    try:
        # None値を-1に変換し、numpy型を通常のPython型に変換してJSON化
        send_data = []
        for x in result_list:
            if x is None:
                send_data.append(-1)
            elif isinstance(x, np.integer):  # numpy整数型の場合
                send_data.append(int(x))
            elif isinstance(x, np.floating):  # numpy浮動小数点型の場合
                send_data.append(float(x))
            elif isinstance(x, datetime.datetime):  # datetime型の場合
                send_data.append(x.isoformat())
            else:
                send_data.append(x)
        socket.send_json(send_data)
    except Exception as e:
